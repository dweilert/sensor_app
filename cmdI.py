    ...
    try:
        ...
        while True:
            ...
            if "l_" in cmd:
                ...
                if qty == 0:
                    print(
                        "Invalid number in logs request. Example: l_17 to get last 17 lines in log.")
                    print("\n" + "---")
                    command_sent = False
                else:
                    data = "logs_" + str(qty)
...
def callMonitor(cmd):
    try:
        info = subprocess.run(["systemctl", cmd, "monitor"],
        ...
        line_number = exception_traceback.tb_lineno
        logger.msg(
            "E", f"callMonitor() Exception type: {exception_type} File name: {filename} Line number: {line_number}")
